...
def _redis_clients():
    try:
        from app import access_token_db as redis_cli  # 复用已连接的 Redis 客户端
        return redis_cli
    except Exception:
        return None
...
def _inject_hust_featured(data: dict, center_lat: float, center_lng: float, radius: int) -> dict:
    """
    若查询中心点在华中科技大学附近（<=6km），则在结果首位插入“华中科技大学心理健康教育中心”。
    - 去重：若已有包含“华中科技大学”字样的条目，则不重复插入。
    - 仅修改本次返回的数据，不改变缓存内容结构（调用者可自行决定是否缓存）。
    """
    try:
        dist = _haversine(center_lat, center_lng, HUST_CENTER['lat'], HUST_CENTER['lng'])
        if dist > 6000:
            return data
    ...
    try:
        ...
        for it in res.get('data', []):
            pois.append({
                ...
                'tel': (it.get('tel') or '').split(';')[0] if it.get('tel') else '',
...
def _amap_search(lat: float, lng: float, radius: int, keyword: str, key: str) -> Optional[dict]:
    ...
    for it in res.get('pois', []):
        ...
        pois.append({'name': name, 'address': addr, 'tel': tel, 'location': {'lat': lat_v, 'lng': lng_v}})
    return {'items': pois}
